...
WebSocketで通信できる内容は、テキストかバイナリデータだけだが、EncoderとDecorderを使うことで任意のオブジェクトを送受信しているかのように実装できる。
...
- クラス名: ChatMessageDecorder
- パッケージ: websocket

ソース編集(ChatMessageDecorder.java)
...
- implements部分を記述
- ChatMessageDecorderの行のクラス名が赤い波線になっているのでそこを選択し、ソース−コードを修正...−すべての抽象メソッドを実装
...
ソース編集(ChatMessageEncorder.java)
...
ChatEndpointでEncoder,Decorderを設定
